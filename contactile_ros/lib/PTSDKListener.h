...
#define MAX_DATAPACKETLEN                                                      \
  1024 // Maximum number of bytes in the data packet (the actual number of bytes
       // may be variable)
...
/** \brief The PTSDKListener class describes a listener for the Communications
 * Hub with a number of PapillArray Tactile Sensors connected.
 *
 * The PTSDKListener class describes a listener for the Communications Hub with
 * a number of PapillArray Tactile Sensors connected.
 *
...
private:
  bool isLogging;
  bool isWaitingForStartByte; // True if PTSDKListener is waiting for the next
                              // start byte; false otherwise
  int waitingForEndByteNum;
  BYTE startBytes[NBYTES_STARTPACKET]; // The bytes to match to a start byte
                                       // from the communications hub
  BYTE endBytes[NBYTES_ENDPACKET];

  BYTE startPacketBuf[NBYTES_STARTPACKET];
  BYTE dataPacketBuf[MAX_DATAPACKETLEN]; // A buffer containing the data packet
                                         // read from the COM port

  int nSensor; // The number of sensors connected to the communications hub
  PTSDKSensor
      *pSensors[MAX_NSENSOR]; // Sensors connected to the communications hub
...
#ifdef _WIN32
  HANDLE
  thread_handle; // The handle to the thread for listening to the COM port
#else
  pthread_t thread_handle;
#endif
  unsigned int threadid; // The id of the thread for listening to the COM port
  bool isListening; // Set to true when the thread begins, and set to false when
  ...
  bool isLogThisSample; // Whether or not to write the current sample to the log
                        // file
...
#ifndef _WIN32
  bool ReadFile(int file_handle, BYTE *buf, DWORD nBytesToRead,
                DWORD *nBytesRead, void *pOverlapped);
  bool WriteFile(int file_handle, char *buf, DWORD nBytesToWrite,
                 DWORD *nBytesWritten, void *pOverlapped);
...
public:
   ...
   */
  int connect(IN const char *portArg, IN int const baudRateArg,
              IN const int parityArg, IN const char byteSizeArg);

  /**
   * Connects to the COM port and starts listening for data (starts the
   * listening thread), processes the data and logs the data to a log file.
   *
   * @param[in] port The COM port name.
   * @param[in] rate The rate of the connection.
   * @param[in] parity The parity of the connection.
   * @param[in] byteSize The byte size for the connection.
   * @param[in] logFileRate The sampling rate for the log file: LOG_RATE_100,
   * LOG_RATE_500 or LOG_RATE_1000
   *
   * @return 0 if successfully connected, error code if unsuccessful.
   */
  int connectAndStartListening(IN const char *port, IN const int rate,
                               IN const int parity, IN const char byteSize,
                               IN const int logFileRate);

  /**
   * The 'infinite' loop of the listening thread.
   * The thread implementation necessitates that this is a public member
   * function. However, this function should never be called except through the
   * startListening function when the listening thread is spawned.
   */
  void run(void);

  /**
   * Reads the next data sample from the COM port and adds the data to the
   * Sensor objects associated with this Listener
   *
  ...
  /**
   * Stops listening for data from the serial port (and kills the listening
   * thread), stops logging data to the log file and disconnects from the COM
   * port.
   */
   ...
   * A bias should be performed after connecting to the serial port and starting
   * to stream data with the sensor unloaded. A bias should be performed each
   * time the sensor is known to be unloaded. A bias operation takes
   * approximately 2 s. Ensure that the sensor remains unloaded throughout this
   * time.
